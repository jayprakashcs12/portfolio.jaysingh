...
export default function Footer({ theme }) {
  ...
  let footClass = `text-3xl rounded-lg md:text-2xl md:px-2 ${padding} hover:scale-105 duration-200 border-2 
  ...
  return (
    <div className={`container ${theme}`}>
      <footer className={`py-12 container ${theme}`}>
        <div className={`max-w-screen-2xl mx-auto px-4 md:px-20 container ${theme}`}>
          <div className="flex flex-col items-center justify-center">
            <div className="flex space-x-4">
              {socialMediaLinks.map((link, index) => (
                <a key={index} href={link.socialURL} target="_blank">
                  <link.socialIcon style={{ fontSize: proSize }} className={footClass} />
                </a>
              ))}
            </div>
